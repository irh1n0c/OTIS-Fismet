...
import { obtenerReportesPaginados, type IBloque, type IReporteIndividual, type IReportesPagination, API_URL } from '../services/api';
...
export const ListadoReportes: React.FC = () => {
  ...
  const [pagination, setPagination] = useState<IReportesPagination | null>(null);
  const limit = 10;
  ...
  const cargarBloques = async (targetPage: number) => {
    try {
      setLoading(true);
      const response = await obtenerReportesPaginados(targetPage, limit);
  ...
  const bloquesFiltrados = bloques
    .map(bloque => {
      ...
      const reportesCoincidentes = bloque.reportes.filter((reporte: IReporteIndividual) =>
  ...
  return (
    <div className="min-h-screen  bg-white-50">
                          ...
                          <CardContent className="p-3 space-y-2">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm gap-2">
                              ...
                              <div className="flex items-center gap-2 mt-2 sm:mt-0">
                                <div className="text-xs text-gray-600">Imagenes: {reporte.imagenesEquipo.length}</div>
                                <Button
                                  size="sm"
                                  variant="outline"
                                ...
                                >
                                  {expandedReports[reporte._id] ? (
                                    <>
                                      <ChevronUp className="mr-1 h-3 w-3" /> Ocultar Imagenes
                                    </>
                                  ) : (
                                    <>
                                      <ChevronDown className="mr-1 h-3 w-3" /> Ver Imagenes
                                    </>
                                  )}
                                </Button>
                              </div>
      ...
      {!loading && !error && pagination && pagination.pages > 1 && (
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-sm text-gray-600">
            Pagina {pagination.page} de {pagination.pages} · Total de bloques: {pagination.total}
          </p>
      ...
      {!loading && !error && bloquesFiltrados.length === 0 && (
        <p className="text-gray-500 mt-8">No se encontraron bloques {filtroMetrologo && 'con reportes para el metrologo especificado'}.</p>
      )}
    </div>
  );
};
